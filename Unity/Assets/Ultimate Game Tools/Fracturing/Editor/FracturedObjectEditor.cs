        ...
        if(bComputeFracturing)
        {
            ...
            else
            {
                ...
                try
                {
					...
					foreach (GameObject cChunkObject in listGameObjects)
					{
						cChunkObject.AddComponent<CNetworkView>();
						cChunkObject.AddComponent<CAsteroidChunkBehaviour>();
					//	cChunkObject.AddComponent<CActorHealth>();
					//	cChunkObject.GetComponent<CActorHealth>().syncNetworkVar = false;
					//	cChunkObject.AddComponent<CActorBreakable>();
					//	cChunkObject.GetComponent<CActorBreakable>().stateTransitions = new float[] { 100 };
					}

					fracturedComponent.SingleMeshObject.transform.localScale = new Vector3(0.99f, 0.99f, 0.99f);

					//fracturedComponent.gameObject.SetActive(false);
                }
