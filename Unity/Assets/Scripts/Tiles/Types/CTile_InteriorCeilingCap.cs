...
public class CTile_InteriorCeilingCap : CTile 
{
	// Member Types
	public enum EType
	{
	...
	// Member Methods
	static CTile_InteriorCeilingCap()
	{
	...
	private void Awake()
	{
		m_TileType = CTile.EType.Interior_Ceiling_Inverse_Corner;
	}
	...
	{
		CTile interiorCeiling = m_TileInterface.GetTile(CTile.EType.Interior_Ceiling);
		
		if(interiorCeiling == null)
			return(false);
		...
		bool leftExemption = interiorCeiling.GetNeighbourExemptionState(leftDir);
		bool rightExemption = interiorCeiling.GetNeighbourExemptionState(rightDir);
		...
		{
			CTile neighbourInteriorCeiling = _Neighbour.m_TileInterface.GetTile(CTile.EType.Interior_Ceiling);
			
			if(neighbourInteriorCeiling != null)
			{
				leftExemption = neighbourInteriorCeiling.GetNeighbourExemptionState(CNeighbour.GetOppositeDirection(leftDir));
				rightExemption = neighbourInteriorCeiling.GetNeighbourExemptionState(CNeighbour.GetOppositeDirection(rightDir));
