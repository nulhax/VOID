...
// Member Methods
    ...
    public GameObject CreateModule(CModuleInterface.EType _eType)
    {
		...
		if(!IsModuleAttached)
		{
	        ...
	        cModuleObject.GetComponent<CNetworkView>().SetEuler(m_Positioner.transform.rotation.eulerAngles);
	...
	void Start()
	{
            ...
            CNetwork.IsServer)
        {
            ...
            if (m_PreplacedModuleBuilt)
                m_cAttachedModuleViewId.Value.GameObject.GetComponent<CModuleInterface>().Build(1.0f);
        }
	}
