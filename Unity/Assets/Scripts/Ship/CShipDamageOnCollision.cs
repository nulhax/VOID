...
public class CShipDamageOnCollision : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (CNetwork.IsServer)
        {
			...
			float theirMass = CUtility.GetMass(collision.gameObject);
			float force = collision.relativeVelocity.magnitude * (theirMass / myMass);
            float impulse = (theirMass * collision.relativeVelocity / (myMass + theirMass)).magnitude;
