...
public class CActorHealth : CNetworkMonoBehaviour
{
	...
	{
		health_internal = _cRegistrar.CreateNetworkVar<float>(OnSyncHealth, float.MinValue);
		state_internal = _cRegistrar.CreateNetworkVar<byte>(OnSyncState, byte.MinValue);
		// Set before Start()
